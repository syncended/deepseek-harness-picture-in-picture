...
.dsh-pip-tool-section { display: grid; grid-template-columns: 28px minmax(0, 1fr); gap: 8px; padding: 9px 10px; }
...
@media (prefers-reduced-motion: reduce) {
  .dsh-pip-launcher, .dsh-pip-dot, .dsh-pip-launcher-badge, .dsh-pip-fish, .dsh-pip-activity-row::after { animation: none !important; transition: none !important; }
    ...
    function ThinkRow({ text, running = false }) {
      ...
      return h(
        ...
        h(
          ...
          h("span", { className: "dsh-pip-activity-title" }, "Think"),
    ...
    function AssistantBlocks({ blocks, running = false, interrupted = false, copy }) {
      ...
      for (let index = 0; index < list.length; index += 1) {
        ...
        if (block.kind === "text" && typeof block.text === "string" && block.text.trim()) {
          ...
          rendered.push(h(ThinkRow, { text: block.text, running: running && index === last, key: `think-${index}` }));
                ...
                ? h(
                    ...
                    input
                      ? h(
                          ...
                          h("span", { className: "dsh-pip-tool-section-label" }, "IN"),
                    ...
                    output
                      ? h(
                          "div",
                          { className: "dsh-pip-tool-section" },
                          h("span", { className: "dsh-pip-tool-section-label" }, "OUT"),
    ...
    function MessageList({ snapshot, copy, session, branchingSeq, onLoadOlder, onBranch }) {
      ...
      }, [snapshot.sessionId, tailKey, pendingSteering.length, runningCalls.length, partialText]);
      ...
      if (projected.length === 0 && pendingSteering.length === 0 && runningCalls.length === 0 && !partialText) {
