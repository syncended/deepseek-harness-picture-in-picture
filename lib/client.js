...
body[data-ds-dark-theme] {
  --dsh-pip-bg: #202124;
  --dsh-pip-bg-soft: #2a2b2f;
  --dsh-pip-bg-user: #303238;
  --dsh-pip-composer: #292a2e;
  --dsh-pip-border: rgba(255, 255, 255, 0.11);
  --dsh-pip-text: #f2f3f5;
  --dsh-pip-muted: #a3a7b0;
...
.dsh-pip-queue-action:focus-visible, .dsh-pip-select:focus-visible, .dsh-pip-control:focus-visible,
...
.dsh-pip-compose textarea:focus-visible {
...
.dsh-pip-header { display: flex; align-items: center; gap: 8px; padding: 12px 12px 10px 16px; border-bottom: 1px solid var(--dsh-pip-border); background: var(--dsh-pip-bg); }
...
.dsh-pip-select { min-width: 0; flex: 1; height: 32px; border: 0; border-radius: 16px; padding: 0 28px 0 10px; color: var(--dsh-pip-text); background: transparent; font: inherit; font-weight: 600; }
.dsh-pip-select:hover { background: var(--dsh-pip-bg-soft); }
...
.dsh-pip-compose { box-sizing: border-box; width: 100%; min-width: 0; padding: 8px 14px 14px; background: linear-gradient(180deg, transparent, var(--dsh-pip-bg) 22%); }
...
.dsh-pip-control, .dsh-pip-control-select { box-sizing: border-box; height: 28px; min-width: 0; border: 0; border-radius: 14px; padding: 0 8px; color: var(--dsh-pip-muted); background: transparent; font: 500 11px/18px Inter, ui-sans-serif, system-ui, sans-serif; cursor: pointer; }
.dsh-pip-control:hover:not(:disabled), .dsh-pip-control-select:hover:not(:disabled) { color: var(--dsh-pip-text); background: var(--dsh-pip-bg-soft); }
...
.dsh-pip-control[data-active="true"] { color: var(--dsh-pip-accent-strong); background: color-mix(in srgb, var(--dsh-pip-accent) 10%, transparent); }
.dsh-pip-context-pill { flex: none; padding-inline: 7px; cursor: default; }
...
.dsh-pip-stats { box-sizing: border-box; width: 100%; display: flex; align-items: center; gap: 7px; padding: 5px 4px 0; overflow-x: auto; color: var(--dsh-pip-muted); white-space: nowrap; scrollbar-width: none; font-size: 9px; line-height: 14px; }
.dsh-pip-stats::-webkit-scrollbar { display: none; }
.dsh-pip-stat { display: inline-flex; align-items: center; gap: 7px; }
.dsh-pip-stat + .dsh-pip-stat::before { content: "|"; color: var(--dsh-pip-border); }
...
@media (max-width: 340px) {
  ...
  .dsh-pip-hero-stage .dsh-pip-compose { padding-inline: 12px; }
}
          ...
          h(
            ...
            h(
              ...
              modelState && modelState !== EMPTY_MODEL_DIRECTORY
                ...
                : null,
              pressure !== null
                ? h("span", { className: "dsh-pip-control dsh-pip-context-pill", title: `${copy.context}: ${pressure}%` }, `${pressure}%`)
                : null,
            ),
            h(
              "button",
          ...
          h(
            ...
            h(
              ...
              groups.map((group) =>
                h(
                  "optgroup",
                  { label: group.label, key: group.key || "no-workspace" },
                  group.rows.map((row) => h("option", { value: row.id, key: row.id }, row.displayTitle || row.title || row.id)),
                ),
              ),
            ),
