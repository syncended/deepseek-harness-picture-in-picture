...
.dsh-pip-icon-button:hover:not(:disabled) { color: var(--dsh-pip-text); background: var(--dsh-pip-bg-soft); }
.dsh-pip-icon-button:disabled { cursor: not-allowed; opacity: .45; }
.dsh-pip-new-chat-button { color: var(--dsh-pip-accent-strong); }
...
.dsh-pip-compose textarea { box-sizing: border-box; width: 100%; min-width: 0; min-height: 32px; max-height: 96px; resize: none; border: 0; outline: 0; padding: 1px 2px 8px; overflow-y: auto; color: var(--dsh-pip-text); background: transparent; font: 15px/1.55 Inter, ui-sans-serif, system-ui, sans-serif; }
...
.dsh-pip-reasoning-selector { max-width: 94px; }
...
.dsh-pip-empty-action { position: relative; z-index: 1; min-height: 36px; display: inline-flex; align-items: center; align-self: center; gap: 7px; margin: 0 auto; border: 0; border-radius: 18px; padding: 7px 14px; color: white; background: var(--dsh-pip-accent); cursor: pointer; font: 600 12px/20px Inter, ui-sans-serif, system-ui, sans-serif; }
.dsh-pip-empty-action:hover:not(:disabled) { background: var(--dsh-pip-accent-strong); }
.dsh-pip-empty-action:disabled { cursor: progress; opacity: .62; }
.dsh-pip-empty-error { position: relative; z-index: 1; max-width: 280px; margin: 10px auto 0; color: var(--dsh-pip-danger); text-align: center; font-size: 11px; line-height: 1.45; }
  ...
  .dsh-pip-launcher, .dsh-pip-dot, .dsh-pip-launcher-badge, .dsh-pip-fish, .dsh-pip-activity-row::after { animation: none !important; transition: none !important; }
    ...
    const COPY = {
      en: {
        ...
        newChat: "New chat",
        newChatBusy: "Creating chat…",
        newChatFailed: "Could not create a new chat.",
        noWorkspace: "Other chats",
        noChats: "No active chats yet.",
        noWorkspaceToStart: "Add a workspace in the main Harness window to start a chat.",
        ...
        reasoningFailed: "Could not change reasoning effort.",
        context: "Context",
        turns: "turns",
        steps: "steps",
        input: "Input",
      ...
      ru: {
        ...
        select: "Выберите чат",
        newChat: "Новый чат",
        newChatBusy: "Создаём чат…",
        newChatFailed: "Не удалось создать новый чат.",
        noWorkspace: "Другие чаты",
        noChats: "Активных чатов пока нет.",
        noWorkspaceToStart: "Добавьте workspace в основном окне Harness, чтобы начать чат.",
        ...
        reasoningFailed: "Не удалось изменить уровень reasoning.",
        context: "Контекст",
        turns: "ходов",
        steps: "шагов",
        input: "Вход",
    ...
    COPY.zh = {
      ...
      newChat: "新建聊天",
      newChatBusy: "正在创建聊天…",
      newChatFailed: "无法创建新聊天。",
      noWorkspace: "其他聊天",
      noChats: "暂无活动聊天。",
      noWorkspaceToStart: "请先在 Harness 主窗口中添加工作区。",
      loadingChats: "正在加载聊天…",
      loading: "正在加载对话…",
      loadOlder: "加载更早的消息",
    ...
    function NewChatGlyph() {
      return h(
        "svg",
        { width: 17, height: 17, viewBox: "0 0 24 24", fill: "none", "aria-hidden": true },
        h("path", { d: "M5 5.5h10a3.5 3.5 0 0 1 3.5 3.5v4A3.5 3.5 0 0 1 15 16.5H9l-4 3v-14Z", stroke: "currentColor", strokeWidth: "1.8", strokeLinejoin: "round" }),
        h("path", { d: "M12 8.5v5M9.5 11h5", stroke: "currentColor", strokeWidth: "1.8", strokeLinecap: "round" }),
      );
    }

    function useSessionsList(sessions) {
      const store = sessions.list;
      // Store methods may use `this`; React invokes these callbacks unbound.
      ...
      return React.useSyncExternalStore(subscribe, getSnapshot, () => EMPTY_LIST);
    }

    function useWorkspacesList(workspaces) {
      const store = workspaces.list;
      const subscribe = React.useCallback((listener) => store.subscribe(listener), [store]);
      const getSnapshot = React.useCallback(() => store.getSnapshot(), [store]);
      return React.useSyncExternalStore(subscribe, getSnapshot, () => EMPTY_WORKSPACES);
    }
    ...
    function reasoningChoicesOf(state, defaultLabel = "Default") {
      const current = state && state.current;
      if (!current) return { value: PROVIDER_DEFAULT_EFFORT, options: [] };
      const group = (state.groups || []).find((item) => item.id === current.provider);
      const model = group && Array.isArray(group.models) ? group.models.find((item) => item.id === current.model) : undefined;
      const reasoning = model && model.reasoning;
      if (!reasoning) return { value: PROVIDER_DEFAULT_EFFORT, options: [] };
      const effective = current.reasoningEffort ?? reasoning.defaultEffort;
      const options = [];
      if (reasoning.defaultEffort === undefined) {
        options.push({ value: PROVIDER_DEFAULT_EFFORT, label: defaultLabel, description: "" });
      }
      for (const effort of Array.isArray(reasoning.efforts) ? reasoning.efforts : []) {
        options.push({
          value: effort.id,
          label: effort.name || displayName(effort.id),
          description: effort.description || "",
        });
      }
      return { value: effective === undefined ? PROVIDER_DEFAULT_EFFORT : effective, options };
    }

    function reasoningSelectionOf(state, value, defaultLabel = "Default") {
      const current = state && state.current;
      const choices = reasoningChoicesOf(state, defaultLabel);
      if (!current || !choices.options.some((option) => option.value === value)) return null;
      return {
        provider: current.provider,
        model: current.model,
        ...(value === PROVIDER_DEFAULT_EFFORT ? {} : { reasoningEffort: value }),
      };
    }
    ...
    function newSessionWorkspaceId(workspaceList, sessionList) {
      const items = workspaceList && Array.isArray(workspaceList.items) ? workspaceList.items : [];
      const current = sessionList && sessionList.current;
      const currentWorkspace = current ? items.find((item) => Array.isArray(item.sessionIds) && item.sessionIds.includes(current)) : undefined;
      return (currentWorkspace && currentWorkspace.workspaceId) || (workspaceList && workspaceList.recentWorkspaceId);
    }

    async function openNewSession(workspaces, sessions) {
      const workspaceId = newSessionWorkspaceId(workspaces.list.getSnapshot(), sessions.list.getSnapshot());
    ...
    function confirmFullAccess(copy, ownerWindow) {
      const target = ownerWindow && !ownerWindow.closed ? ownerWindow : typeof window !== "undefined" ? window : null;
      if (!target || typeof target.confirm !== "function") return false;
      return target.confirm(copy.fullAccessConfirm);
    }
                      ...
                      return h(
                        "button",
                        {
                          ...
                          onClick: () => {
                            setOpen(false);
                            rootRef.current?.querySelector(".dsh-pip-control-select")?.focus();
                            if (!active) void onSelect(option.value, rootRef.current?.ownerDocument?.defaultView);
                          },
                        },
                        h(
                          "span",
    ...
    function Composer({ copy, draft, attachments, imageEnabled, session, snapshot, structuredPending, sending, cancelling, sendError, canSend, permissions, plan, modelState, contextPressure, sessionStats, tokenUsage, controlsBusy, onPermission, onPlan, onModel, onReasoning, onAddImages, onRemoveImage, onDraft, onSend, onCancel }) {
      ...
      const modelValue = modelState && modelState.current
        ...
        : "";
      const reasoningChoices = reasoningChoicesOf(modelState, copy.reasoningDefault);
          ...
          h(
            ...
            h(
              ...
              modelState && modelState !== EMPTY_MODEL_DIRECTORY
                ...
                : null,
              reasoningChoices.options.length
                ? h(ComposerSelector, {
                    kind: "reasoning",
                    value: reasoningChoices.value,
                    label: copy.reasoning,
                    disabled: controlsLocked || modelState.status === "loading" || modelState.status === "selecting",
                    options: reasoningChoices.options,
    ...
    function MiniChat({ sessions, workspaces, modelDirectories, localeId, onClose, focusMain, notice }) {
      const copy = copyForLocale(localeId);
      const list = useSessionsList(sessions);
      const workspaceList = useWorkspacesList(workspaces);
      ...
      const [startSessionError, setStartSessionError] = React.useState("");
      ...
      const newSessionTarget = newSessionWorkspaceId(workspaceList, list);
      const canStartSession = Boolean(newSessionTarget && workspaceList.baselinesReady && !startingSession);
      ...
      const startNewSession = async () => {
        ...
        try {
          await openNewSession(workspaces, sessions);
        } catch (error) {
          setStartSessionError(errorMessage(error, copy.newChatFailed));
        } finally {
      ...
      const changePermission = async (value, ownerWindow) => {
        if (!permissions || !permissions.options.some((option) => option.value === value && value !== "custom")) return;
        if (value === "danger-full-access" && !confirmFullAccess(copy, ownerWindow)) return;
      ...
      const changeReasoning = async (value) => {
        const selection = reasoningSelectionOf(modelState, value, copy.reasoningDefault);
        if (!modelDirectory || !selection || controlsBusy) return;
        const targetId = selectedId;
        setControlsBusy("reasoning");
        setSendErrors((state) => ({ ...state, [targetId]: "" }));
        try {
          await modelDirectory.select(selection);
        } catch (error) {
          setSendErrors((state) => ({ ...state, [targetId]: errorMessage(error, copy.reasoningFailed) }));
        } finally {
          setControlsBusy((busy) => (busy === "reasoning" ? null : busy));
        }
      };
        ...
        return h(
          ...
          h(
            ...
            h(
              ...
              h("button", { type: "button", className: "dsh-pip-icon-button dsh-pip-new-chat-button", disabled: !canStartSession, title: startingSession ? copy.newChatBusy : copy.newChat, "aria-label": copy.newChat, onClick: () => void startNewSession() }, h(NewChatGlyph)),
            ...
            h(
              ...
              h(
                ...
                h(HeroHeading, { copy }),
                h("div", { className: "dsh-pip-no-sessions", role: "status" }, list.phase === "ready" ? newSessionTarget ? copy.noChats : copy.noWorkspaceToStart : copy.loadingChats),
                newSessionTarget
                  ? h("button", { type: "button", className: "dsh-pip-empty-action", disabled: !canStartSession, onClick: () => void startNewSession() }, h(NewChatGlyph), startingSession ? copy.newChatBusy : copy.newChat)
                  : null,
                startSessionError ? h("div", { className: "dsh-pip-empty-error", role: "alert" }, startSessionError) : null,
              ),
            ),
          ),
        );
      ...
      return h(
        ...
        h(
          ...
          h(
            ...
            notice || startSessionError ? h("span", { className: "dsh-pip-notice", title: startSessionError || notice, "aria-label": startSessionError || notice }, "ⓘ") : null,
            h("button", { type: "button", className: "dsh-pip-icon-button dsh-pip-new-chat-button", disabled: !canStartSession, title: startingSession ? copy.newChatBusy : copy.newChat, "aria-label": copy.newChat, onClick: () => void startNewSession() }, h(NewChatGlyph)),
    ...
    function PictureInPictureEntry({ sessions, workspaces, modelDirectories, locale }) {
      ...
      if (inlineOpen) {
        children.push(
          h(
            ...
            h(MiniChat, { sessions, workspaces, modelDirectories, localeId, onClose: close, focusMain, notice }),
          ),
        );
      }
      if (surface && !surface.win.closed) {
        children.push(ReactDOM.createPortal(h(MiniChat, { sessions, workspaces, modelDirectories, localeId, onClose: close, focusMain, notice }), surface.root, "native-pip"));
      }
        ...
        () =>
          ctx.slots.inject("shell.overlay", () =>
            ctx.slots.register(
              ...
              () => h(PictureInPictureEntry, { sessions: ctx.sessions, workspaces: ctx.workspaces, modelDirectories: ctx.modelDirectories, locale: ctx.locale }),
            ),
          ),
    ...
    exports.__testing = { useSessionsList, useWorkspacesList, copyForLocale, languageForLocale, runningTurnStartTime, formatRunDuration, textOfContent, imagesOfContent, textOfAssistant, answerTextOfAssistant, projectNode, visibleConversationRows, displayName, modelChoicesOf, reasoningChoicesOf, reasoningSelectionOf, contextPercent, statsSegments, todoSummary, sessionRows, sessionGroups, newSessionWorkspaceId, openNewSession, confirmFullAccess, stateOf };
